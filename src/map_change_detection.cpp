																		...
																		enough_time_elapsed(false), w_idle(false)
{
    ...
    update_localization = nh_.serviceClient<std_srvs::Empty>("/lost_localization");
    stop_robot = nh_.serviceClient<std_srvs::Empty>("/publish_cmd_vel/stop");
	call_service = true;
...
void MapChangeDetection::detectChanges() {
	...
	std::cout<<"anom, value: " << anomalous_idxs.size()<<","<<(max_anom_beam_frac * scan_handler->getValidBeams())<<std::endl;
	if (anomalous_idxs.size() > (max_anom_beam_frac * scan_handler->getValidBeams())) {
		...
		//std::cout<<"chiama server: "<<std::endl;
        ...
        if(call_service)
		{
				...
				if (update_localization.call(req_srv))
				{
					
				}
				else  ROS_ERROR("Failed to call service updateloc in merger_manager");
				
			
			if (stop_robot.call(req_srv))
        	{
        	}
			else  ROS_ERROR("Failed to call service stop robot ");
